...
def get_dataset_path(dataset='pancreas'):
    files = ['train_lab.txt', 'train_unlab.txt', 'test.txt', 'train_whole.txt']
    return ['/'.join(['data_lists_cora', dataset, f]) for f in files]


class Pancreas(Dataset):
    """ Pancreas Dataset """

    def __init__(self, base_dir, name, split, no_crop=False, TTA=False, require_mask=False):
        ...
        elif split == 'train_whole':
            data_path = data_list_paths[3]
            self.transform = tr_transform
        else:
